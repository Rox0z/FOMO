...
export class HomeComponent implements OnInit {
  ...
  private mapApiEvent(event: any, index: number, fallbackImages: string[]): EventItem {
    ...
    return {
      ...
      image: event.bannerUrl || this.imageForEvent(event.name, index, fallbackImages),
      featured: index === 0,
  ...
  private imageForEvent(name: string, index: number, fallbackImages: string[]): string {
    const key = name
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
      .toLowerCase()
    ...
    return eventImages[key] || fallbackImages[index % fallbackImages.length] || fallbackImages[0];
  }
