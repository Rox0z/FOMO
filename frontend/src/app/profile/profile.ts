...
export class ProfileComponent implements OnInit {
  ...
  initForm(): void {
    if (this.user) {
      this.profileForm = this.fb.group({
        ...
        countryCode: [this.user.countryCode || '+351', [Validators.required]]
      });
    }
  }
 ...
 @HostListener('document:click', ['$event'])
  ...
  getCountryName(code: string): string {
    const countries: any = { '+351': 'Portugal', '+55': 'Brasil', '+34': 'Espanha', '+1': 'USA', US: 'USA', PT: 'Portugal', BR: 'Brasil', ES: 'Espanha' };
    return countries[code] || countries[code?.toUpperCase()] || code || 'Portugal';
  }
