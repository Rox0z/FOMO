...
export class EventsService {
  ...
  // -------------------------
  // PUBLIC - FIND ONE APPROVED EVENT
  // Non-approved events are only accessible to admins (via findOneAdmin)
  // -------------------------
  async findOne(id: number) {
    const event = await this.db.query.events.findFirst({
      where: and(eq(events.id, id), eq(events.status, 'approved')),
    });
  ...
  // -------------------------
  // UPDATE EVENT DIRECTLY (ADMIN ONLY)
  // Vendors must use the request-edit workflow.
  // -------------------------
  async update(id: number, dto: UpdateEventDto, user: any) {
    ...
    if (user.role !== 'admin') {
      throw new ForbiddenException(
        'Direct event updates are restricted to admins. Vendors must use the request-edit workflow.',
      );
    }
